...
[Script]
WeTalk抓包 = script-request-header, ^https:\/\/api\.wetalkapp\.com\/app\/queryBalanceAndBonus, tag=WeTalk抓包, script-path=本地文件路径/WeTalk.js, requires-body=false
WeTalk签到 = script, cron "20 8,20 * * *", tag=WeTalk签到, script-path=本地文件路径/WeTalk.js, timeout=60
...
// 【Loon适配】替换 $prefs 为 $persistentStore
function loadStore() {
...
// 【Loon适配】替换 $notify 为 $notification.post
...
function runAccount(acc, index, total) {
  ...
  // 【Loon适配】替换 $task.fetch 为 $httpClient.get
